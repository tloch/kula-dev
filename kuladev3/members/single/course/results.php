...
if(isset($_GET['action']) && !is_numeric($_GET['action'])){
		echo '<div id="message"><p>'.__('Invalid Quiz','vibe').'</p></div>';
}else{
	...
	if(count($questions)){
		...
		if(isset($retakes) && $retakes){
			...
			if(($retakes - count($quiz_retakes)) > 0){
				...
				echo '<p>'.__('Number of retakes avaialble ','vibe').' : <strong>'.count($quiz_retakes).__(' out of ','vibe').$retakes.'</strong></p>';
...
<li><i class="icon-task"></i>
	...
	echo '<i class="icon-clock"></i> '.__('Submitted ','vibe').tofriendlytime($diff) .' ago';
